...
starship_precmd() {
    ...
    if [[ ! -z "${STARSHIP_START_TIME+1}" ]]; then
        ...
        PROMPT="$("/usr/bin/starship" prompt --status=$STATUS --cmd-duration=$STARSHIP_DURATION --jobs="$NUM_JOBS")"
        unset STARSHIP_START_TIME
    else
        PROMPT="$("/usr/bin/starship" prompt --status=$STATUS --jobs="$NUM_JOBS")"
    fi
}
starship_preexec(){
...
{
    PROMPT=$("/usr/bin/starship" prompt --keymap=$KEYMAP --jobs="$(jobs | wc -l)")
    zle reset-prompt
}
